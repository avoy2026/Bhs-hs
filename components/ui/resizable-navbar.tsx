...
export const Navbar = ({ children, className }: NavbarProps) => {
  ...
  return (
    <motion.div
      // Fixed floating navbar — shrinks into a pill on scroll
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        visible
          ? "bg-white/80 text-neutral-800 dark:bg-neutral-950/80"
          : "bg-black/25 text-white [&_a]:text-white [&_span]:text-white",
        className,
      )}
    >
      {children}
      ...
      {items.map((item, idx) => (
        ...
        >
          {hovered === idx && (
            <motion.div
              layoutId="hovered"
              className="absolute inset-0 h-full w-full rounded-full bg-gray-100/80 dark:bg-neutral-800"
            />
          )}
