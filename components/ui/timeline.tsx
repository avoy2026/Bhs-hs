...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  useMotionValueEvent(heightTransform, "change", (lineHeight) => {
    const next = itemRefs.current.map((el) => {
      if (!el) return false;
      return lineHeight >= el.offsetTop + 20;
    });
    setReached((prev) => {
      if (
  ...
  return (
    <div
      className="w-full bg-[var(--brand-fog)] font-sans md:px-10"
      ref={containerRef}
    >
      <div className="max-w-7xl mx-auto py-20 px-4 md:px-8 lg:px-10">
        ...
        <p className="max-w-lg text-sm text-[var(--muted-foreground)] md:text-base">
          ...
          in learning, in values, and in the success of our students.
        </p>
      </div>
              ...
              <div className="h-10 absolute left-3 md:left-3 w-10 rounded-full bg-white flex items-center justify-center">
                <div
                  className={cn(
                    ...
                    reached[index]
                      ? "border-blue-400 bg-blue-500 shadow-[0_0_0_4px_rgb(59_130_246/25%)]"
