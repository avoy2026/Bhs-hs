...
export default function TestimonialCard({
    ...
    return (
      <article
        className={cn(
          ...
          highlight && "ring-1 ring-[var(--brand-gold)]/25",
          className
        )}
      >
        <div className="relative h-24 w-24 shrink-0 mx-auto lg:mx-0">
          {image ? (
            ...
            <div className="h-full w-full rounded-2xl bg-[var(--brand-mist)] inline-flex items-center justify-center ring-2 ring-white shadow-sm">
              <IconUser className="h-10 w-10 text-[var(--brand-navy)]/50" stroke={1.6} />
            </div>
          )}
        </div>
        <div>
          ...
          <blockquote className="relative mt-4 text-base md:text-lg leading-relaxed text-[var(--foreground)]/85">
            <IconQuote className="absolute -left-0.5 -top-1 h-5 w-5 text-[var(--brand-gold)]/50" stroke={1.8} />
            <span className="pl-6">{quote}</span>
          </blockquote>
          <div className="mt-6 pt-5 border-t border-[var(--border)]">
            <p className="font-display font-semibold text-lg text-[var(--brand-navy)]">{name}</p>
            <p className="mt-0.5 text-sm text-[var(--muted-foreground)]">{role}</p>
          </div>
        </div>
      </article>
    );
  ...
  if (variant === "compact") {
    return (
      ...
      >
        ...
        <blockquote className="relative flex-1 text-sm leading-relaxed text-[var(--foreground)]/85">
          <IconQuote className="absolute -left-0.5 -top-1 h-4 w-4 text-[var(--brand-gold)]/50" />
          <span className="pl-5">{quote}</span>
        </blockquote>
        <div className="mt-5 pt-4 border-t border-[var(--border)] flex items-center gap-3">
          <div className="relative h-10 w-10 shrink-0 rounded-full overflow-hidden bg-[var(--brand-mist)] ring-1 ring-black/5">
            {image ? (
              <Image src={image} alt={`${name}`} fill className="object-cover" />
            ) : (
              <div className="h-full w-full inline-flex items-center justify-center">
                <IconUser className="h-5 w-5 text-[var(--brand-navy)]/45" stroke={1.6} />
              </div>
            )}
          </div>
          <div className="min-w-0">
            <p className="font-semibold text-[0.92rem] text-[var(--brand-navy)] truncate">{name}</p>
            <p className="text-[0.75rem] text-[var(--muted-foreground)] truncate">{role}</p>
          </div>
        </div>
      </article>
    );
  ...
  return (
    ...
    >
      <IconQuote className="absolute right-6 top-6 h-10 w-10 text-[var(--brand-gold)]/15" stroke={1.4} />
      ...
      <blockquote className="relative flex-1 text-[0.95rem] leading-relaxed text-[var(--foreground)]/85">
        <span className="relative z-10">{quote}</span>
      </blockquote>
      <div className="mt-6 pt-5 border-t border-[var(--border)] flex items-center gap-3.5">
        <div className="relative h-12 w-12 shrink-0 rounded-2xl overflow-hidden bg-[var(--brand-mist)] ring-1 ring-black/5">
          {image ? (
            <Image src={image} alt={`${name}`} fill className="object-cover" />
          ) : (
            <div className="h-full w-full inline-flex items-center justify-center">
              <IconUser className="h-6 w-6 text-[var(--brand-navy)]/45" stroke={1.6} />
            </div>
          )}
        </div>
        <div className="min-w-0">
          <p className="font-display font-semibold text-[1.02rem] text-[var(--brand-navy)]">{name}</p>
